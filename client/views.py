...
def client_index(request):
    uid = Client.objects.get(email=request.session['email'])
...
def view_service(request,pk):
    ...
    try:
        uids = am.User.objects.get(email=request.session['email'])
...
def client_signin(request):
    ...
    if request.method == 'POST':
        try:
            uid = Client.objects.get(email=request.POST['email'])
            if request.POST['password'] == uid.password:
                request.session['client'] = request.POST['email']
                return redirect('client_index')
                # return render(request,'client-index.html',{'uid':uid,'services':services})
            return render(request,'client-sign-in.html',{'msg':'Pasword is incorrect'})
        except:
            return render(request,'client-sign-in.html',{'msg':'Acccount does not exists'})
    return render(request,'client-sign-in.html')
...
def client_signup(request):
    if request.method == "POST":
        try:
            ...
            return render(request,'client-sign-up.html',{'msg':msg})
        except:
            ...
            return render(request,'client-sign-up.html',{'msg':'Both are not same'})
...
def client_otp(request):
    if request.POST['uotp'] == request.POST['otp']:
        ...
        del temp
        return render(request,'client-sign-in.html',{'msg':'Account Created'})
...
def client_forgot(request):
    if request.method == 'POST':
        try:
            ...
            uid.save()
            return render(request,'client-sign-in.html',{'msg':'Password sent on your mail....'})
...
def add_client_details(request):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            uid.save()
        return render(request, 'add-client-details.html',{'uid':uid,'msg':'Profile has been updated'})
    except:
        return render(request, 'add-client-details.html',{'uid':uid})

def book_prod(request,pk):
    uid = Client.objects.get(email=request.session['client'])
    try:
        client = Client.objects.get(email=request.session['client'])
        if request.method == 'POST':
            service = Service.objects.get(id=pk)
            book = Booking.objects.create(
                service = service,
                book_date = request.POST['date'],
                address = request.POST['address'],
                client = client,
                amount = service.price,
            )
            currency = 'INR'
            amount = int(service.price)*100  # Rs. 200
...
#                 address = request.POST['address'],
#                 client = client,
#                 amount = service.min_charge
#                 )
#             currency = 'INR'
#             amount = int(service.min_charge)*100  # Rs. 200
...
#             context['callback_url'] = callback_url
#             context['book'] = book
#             return render(request,'book_proceed.html',{'client':client},context=context)
#         return render(request,'book-prod.html')
#     except:
#         return redirect('client_signin')
